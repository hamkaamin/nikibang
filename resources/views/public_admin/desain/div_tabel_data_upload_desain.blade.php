...
<div class="row">
    <div class="col-md-12">  
        <button type="button" class="btn btn-primary" onclick="$('#frmUpload_desain').toggle(500);">
...
<form style="display: none;" id="frmUpload_desain" class="form-horizontal"  enctype="multipart/form-data">
<div class="modal-body">
    <div class="form-group">
        {{ csrf_field() }}
    </div>  
    <input type="hidden" name="desain_id" value="{{$id}}">
    <div class="form-group">
...
<div class="modal-footer">
    ...
    <button type="submit" class="btn btn-primary btn_data_upload_desain"><i class="icon-check"></i> Simpan</button>
</div>
</form>
<table class="table datatable-basic">
    ...
    <tbody>
        @php $no=0; @endphp
        @foreach ($data_upload_desain as $r)
        @php $no++; @endphp
        <tr>
            ...
            <td class="tCenter">
                ...
                if ($r['status'] == 0) {
                    ?>
                    <button title="hapus" type="button" class="btn btn-danger btn-icon btn-rounded btn-xs" onclick="hapus_upload_data_desain('{{csrf_token()}}', '{{$r->id}}', '{{$id}}', '#div_tabel_data_upload_desain');"><i
    ...
    <script type="text/javascript">
        ...
        $('#frmUpload_desain').on('submit', function (event) {
            event.preventDefault(); 
            $.ajax({
                url: "{{ route('upload_desain.action') }}",
                method: "POST",
                    ...
                    if(data.res == 'success'){ 
                        ...
                        function(isConfirm){ 
                            if(isConfirm){ 
                                ...
                                div_tabel_data_upload_desain('{{csrf_token()}}', '#div_tabel_data_upload_desain', '{{$id}}');
                            }
                        });
                    } else {
