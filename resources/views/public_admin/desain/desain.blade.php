	...
	<div class="col-lg-12">
		...
		<div class="panel panel-indigo">
			<div class="panel-heading">
				<h6 class="panel-title">Data Desain Industri </h6>
			...
			<div class="panel-body">
				<div>
					<a class="btn btn-info" onclick="tambah_modal_desain('{{csrf_token()}}', '#ModalBiru')">Tambah Data <i class="icon-plus3 position-right"></i></a>
				</div>
				<br>
				<div class="">
					<table class="table table-bordered table-hover datatable-basic">
						...
						<tbody>
							...
							@if($r['status'] == 1)
								<?php $status = '(PA) Diberi Hak Desain'; ?>
							@elseif($r['status']==2)
                        ...
                        <ul class="dropdown-menu">
                            <li> 
									...
									edit_modal_desain('{{csrf_token()}}', '{{ $id }}', '#ModalTeal')
									" id="modal_update_desain" title="Ubah" class="btn bg-teal-400 btn-xs btn-block">
									<i class="glyphicon glyphicon-edit"></i>Ubah</button>  
                            </li>
                            <li> 
									<button type="button" onclick="
									detail_modal_desain('{{csrf_token()}}', '{{ $id }}', '#ModalHijau')
									" id="modal_update_desain" title="Detail" class="btn bg-green btn-xs btn-block">
									<i class="glyphicon glyphicon-repeat"></i>Detail</button>  
                            </li>
                            <li>
                                	<button type="button" onclick="
									modal_isi_desain('{{csrf_token()}}', '{{ $id }}', '#ModalKuning')
									" id="modal_update_desain" title="Isi Data" class="btn bg-warning btn-xs btn-block">
									<i class="glyphicon glyphicon-search"></i>Isi Data</button>
                            </li> 
                            <li>
                            	<button type="button" onclick="hapus_data_desain('{{csrf_token()}}','{{ $id }}')"
