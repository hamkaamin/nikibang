	...
	<div class="col-lg-12">
		...
		<div class="panel panel-indigo">
			<div class="panel-heading">
				<h6 class="panel-title">Data Ki Komunal </h6>
				...
				<div class="">
					<table class="table table-bordered table-hover datatable-basic">
						...
						<tbody>
							...
							$verifikasi = '';
							?>
							@foreach ($data_ki_komunal_belum_biaya as $r) 
							@php
							$no ++;
							$id = $r['id'];
							...
							<tr>
								...
								@if(Auth::user()->role_id == 1)
								<td><span onclick="
									modal_verif_ki_komunal_belum_biaya('{{csrf_token()}}', '{{ $id }}', '#ModalTealSm')" style="cursor: pointer;" class="label label-default label-rounded">Belum Diverifikasi</span></td>
                        ...
                        <ul class="dropdown-menu">
                             {{-- <li> 
									...
									edit_modal_ki_komunal('{{csrf_token()}}', '{{ $id }}', '#ModalTeal')
									" id="modal_update_ki_komunal" title="Ubah" class="btn bg-teal-400 btn-xs btn-block">
									<i class="glyphicon glyphicon-edit"></i>Ubah</button>  
                            </li> --}}
                            <li> 
								<button type="button" onclick="
								detail_modal_ki_komunal('{{csrf_token()}}', '{{ $id }}', '#ModalHijau')
								" id="modal_update_ki_komunal" title="Detail" class="btn bg-green btn-xs btn-block">
								<i class="glyphicon glyphicon-repeat"></i>Detail</button>  
						</li>
						<li>
							<button type="button" onclick="hapus_data_ki_komunal('{{csrf_token()}}','{{ $id }}')"
