...
<div class="row">
    <div class="col-md-12">  
        <button type="button" class="btn btn-primary" onclick="$('#frmUpload_paten').toggle(500);">
...
<form style="display: none;" id="frmUpload_paten" class="form-horizontal"  enctype="multipart/form-data">
<div class="modal-body">
    <div class="form-group">
        {{ csrf_field() }}
    </div>  
    <input type="hidden" name="paten_id" value="{{$id}}">
    <div class="form-group">
...
<div class="modal-footer">
    ...
    <button type="submit" class="btn btn-primary btn_data_upload_paten"><i class="icon-check"></i> Simpan</button>
</div>
</form>
<table class="table datatable-basic">
    ...
    <tbody>
        @php $no=0; @endphp
        @foreach ($data_upload_paten as $r)
        @php $no++; @endphp
        <tr>
            ...
            <td class="tCenter">
                ...
                if ($r['status'] == 0) {
                    ?>
                    <button title="hapus" type="button" class="btn btn-danger btn-icon btn-rounded btn-xs" onclick="hapus_upload_data_paten('{{csrf_token()}}', '{{$r->id}}', '{{$id}}', '#div_tabel_data_upload_paten');"><i
    ...
    <script type="text/javascript">
        ...
        $('#frmUpload_paten').on('submit', function (event) {
            event.preventDefault(); 
            $.ajax({
                url: "{{ route('upload_paten.action') }}",
                method: "POST",
                    ...
                    if(data.res == 'success'){ 
                        ...
                        function(isConfirm){ 
                            if(isConfirm){ 
                                ...
                                div_tabel_data_upload_paten('{{csrf_token()}}', '#div_tabel_data_upload_paten', '{{$id}}');
                            }
                        });
                    } else {
