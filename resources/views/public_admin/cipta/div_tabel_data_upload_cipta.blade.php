...
<div class="row">
    <div class="col-md-12">  
        <button type="button" class="btn btn-primary" onclick="$('#frmUpload_cipta').toggle(500);">
...
<form style="display: none;" id="frmUpload_cipta" class="form-horizontal"  enctype="multipart/form-data">
<div class="modal-body">
    <div class="form-group">
        {{ csrf_field() }}
    </div>  
    <input type="hidden" name="cipta_id" value="{{$id}}">
    <div class="form-group">
...
<div class="modal-footer">
    ...
    <button type="submit" class="btn btn-primary btn_data_upload_cipta"><i class="icon-check"></i> Simpan</button>
</div>
</form>
<table class="table datatable-basic">
    ...
    <tbody>
        @php $no=0; @endphp
        @foreach ($data_upload_cipta as $r)
        @php $no++; @endphp
        <tr>
            ...
            <td class="tCenter">
                ...
                if ($r['status'] == 0) {
                    ?>
                    <button title="hapus" type="button" class="btn btn-danger btn-icon btn-rounded btn-xs" onclick="hapus_upload_data_cipta('{{csrf_token()}}', '{{$r->id}}', '{{$id}}', '#div_tabel_data_upload_cipta');"><i
    ...
    <script type="text/javascript">
        ...
        $('#frmUpload_cipta').on('submit', function (event) {
            event.preventDefault(); 
            $.ajax({
                url: "{{ route('upload_cipta.action') }}",
                method: "POST",
                    ...
                    if(data.res == 'success'){ 
                        ...
                        function(isConfirm){ 
                            if(isConfirm){ 
                                ...
                                div_tabel_data_upload_cipta('{{csrf_token()}}', '#div_tabel_data_upload_cipta', '{{$id}}');
                            }
                        });
                    } else {
