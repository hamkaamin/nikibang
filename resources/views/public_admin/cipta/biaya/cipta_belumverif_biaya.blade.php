	...
	<div class="col-lg-12">
		...
		<div class="panel panel-indigo">
			<div class="panel-heading">
				<h6 class="panel-title">Data Cipta </h6>
			...
			<div class="panel-body">
				...
				<div>
					<a class="btn btn-info" onclick="tambah_modal_cipta('{{csrf_token()}}', '#ModalBiru')">Tambah Data <i class="icon-plus3 position-right"></i></a>
				</div>
				@endif
				<br>
				<div class="">
					<table class="table table-bordered table-hover datatable-basic">
						...
						<tbody>
							...
							$verifikasi = '';
							?>
							@foreach ($data_cipta_belum_biaya as $r) 
							@php
							$no ++;
							$id = $r['id'];
							...
							@elseif ($r->is_biaya === null) 
								<?php $verifikasi = '<span class="label label-default label-rounded">Belum Diverifikasi</span>'; ?> 
							@endif
							<tr>
								<td>{{$no}}</td> 
								<td>{{$r['nama']}}</td>   
								...
								@if(Auth::user()->role_id == 1)   
								<td><span onclick="
									modal_verif_cipta_belum_biaya('{{csrf_token()}}', '{{ $id }}', '#ModalTealSm')" style="cursor: pointer;" class="label label-default label-rounded">Belum Diverifikasi</span></td>  
								@elseif(Auth::user()->role_id == 3)
								<td><span class="label label-default label-rounded">Belum Diverifikasi</span></td>  
								@endif 
								@if(Auth::user()->role_id == 1)
								<td> 
                        ...
                        <ul class="dropdown-menu">
                            <li> 
									...
									edit_modal_cipta('{{csrf_token()}}', '{{ $id }}', '#ModalTeal')
									" id="modal_update_cipta" title="Ubah" class="btn bg-teal-400 btn-xs btn-block">
									<i class="glyphicon glyphicon-edit"></i>Ubah</button>  
                            </li>
                            <li> 
									<button type="button" onclick="
									detail_modal_cipta('{{csrf_token()}}', '{{ $id }}', '#ModalHijau')
									" id="modal_update_cipta" title="Detail" class="btn bg-green btn-xs btn-block">
									<i class="glyphicon glyphicon-repeat"></i>Detail</button>  
                            </li>
                            <li>
                                	<button type="button" onclick="
									modal_isi_cipta('{{csrf_token()}}', '{{ $id }}', '#ModalKuning')
									" id="modal_update_cipta" title="Isi Data" class="btn bg-warning btn-xs btn-block">
									<i class="glyphicon glyphicon-search"></i>Isi Data</button>
                            </li> 
                            <li>
                            	<button type="button" onclick="hapus_data_cipta_belum_biaya('{{csrf_token()}}','{{ $id }}')"
